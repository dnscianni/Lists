 ...
 * 
 */
public class UnboundedList<K extends Comparable<K>, V> implements List<K, V> {
	...
	public UnboundedList(int initialSize) {
		first = 0;
		last = 0;
	...
	public boolean add(K key, V value) {
		if ((last - first + 1) % theList.length == 0) {
			resizeList();
		}
	...
	private void resizeList() {
		...
		Item<K, V>[] tempList = new Item[(int) (theList.length * 1.5)];
		for (int i = 0, tempIndex = first; i < theList.length; i++, tempIndex = (tempIndex + 1)
				% theList.length) {
			tempList[i] = theList[tempIndex];
		}
		first = 0;
